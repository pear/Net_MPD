...
class Net_MPD_Common
{
  ...
  public function connect()
	...
	if (!is_null($this->connection_params['password'])) {
	  if ($this->runCommand('password', $this->connection_params['password']) == self::ACK_PASSWORD) {
	    throw new PEAR_Exception('Password invalid.',
				     self::ACK_PASSWORD);
	  }
	}
	return true;
      ...
      throw new PEAR_Exception('Error connecting: '.$errn.' ; '.$errs,
  ...
  function write($data)
	...
	throw new PEAR_Exception('Write failed',
				 self::WRITE_FAILED);
      }
  ...
  function read()
    {
      //Are we connected?
      if (!$this->isConnected()) {
	throw new PEAR_Exception('Not connected',
				 self::CONNECTION_NOT_OPENED);
      }
	...
	if (preg_match('/^ACK \[(.*?)\@(.*?)\] \{(.*?)\} (.*?)$/', $line, $matches)) {
	  ...
	  $this->_current_error =  array(
					 'ack' => $matches[1],
					 'func' => $matches[3],
					 'error' => $matches[4]
					 );
	  throw new PEAR_Exception('Command Failed',
				   self::ACK_COMMAND_FAILED);
